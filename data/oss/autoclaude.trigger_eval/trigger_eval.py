...
def _run_llm_tmeoa(query: str, model: str | None, timeout: int, cwd: str) -> str:
    """tmeoa backend for the LLM probe (LLM_BACKEND=tmeoa).

    Replaces the `claude -p` subprocess with a direct call to the tmeoa
    OpenAI-chat-compatible gateway. A plain chat model has no Claude agent
    tool-use, so we wrap the reply as a single assistant text event; downstream
    tool-use parsing then correctly reports "no Skill trigger" (a miss), which is
    the expected outcome for a non-agentic endpoint.
    """
    import sys as _sys

    client_dir = os.environ.get("TMEOA_CLIENT_DIR")
    if client_dir:
        _sys.path.insert(0, client_dir)
    from tmeoa import client as _tmeoa

    chosen = model or os.environ.get("LLM_MODEL", "deepseek-v4-flash-official")
    text = _tmeoa.chat_text(query, model=chosen, max_tokens=2000, strict=False)
    event = {
        "type": "assistant",
...
def run_probe(query: str, model: str | None, timeout: int, cwd: str) -> str:
    """One probe; returns raw stream-json text. Raises RuntimeError
    ...
    With ``LLM_BACKEND=tmeoa`` the probe is served by the tmeoa gateway instead
    of the ``claude -p`` CLI (see _run_llm_tmeoa)."""
    if os.environ.get("LLM_BACKEND") == "tmeoa":
        return _run_llm_tmeoa(query, model, timeout, cwd)
